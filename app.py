...
"""
불량율 분석
선택한 폴더의 PDF 파일을 읽어 하나의 엑셀 파일로 통합 저장하는 GUI 프로그램입니다.
...
def process_uploaded_files(uploaded_files):
    """Streamlit 업로드 파일들을 임시 폴더에 저장한 뒤 분석합니다."""
    records = []
    failed_files = []
    logs = []
    total_pdf_count = len(uploaded_files)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        logs.append(f"{success_count} files are successfully loaded.")
        logs.append(f"{failed_count} files are not loaded correctly.")
        ...
        excel_bytes = None
        output_filename = f"불량율_분석_통합_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        if records:
            output_path = os.path.join(tmpdir, output_filename)
            make_workbook(records, output_path)
...
def run_streamlit_app():
    ...
    st.write("PDF 제품평가보고서 여러 개를 업로드하면 하나의 엑셀 파일로 통합합니다.")

    uploaded_files = st.file_uploader(
        "PDF 파일 선택",
        type=["pdf"],
        accept_multiple_files=True,
        help="동일 또는 유사 양식의 제품평가보고서 PDF 파일을 여러 개 선택할 수 있습니다.",
    )

    if uploaded_files:
        st.info(f"선택된 PDF 파일: {len(uploaded_files)}개")

    run_button = st.button("분석 실행", type="primary", disabled=not uploaded_files)

    if run_button:
        with st.spinner("PDF 파일을 분석하는 중입니다..."):
            result = process_uploaded_files(uploaded_files)

        st.subheader("처리 결과 로그")
        st.text_area("log", "\n".join(result["logs"]), height=360, label_visibility="collapsed")
        ...
        col1.metric("전체 PDF", total_count)
        ...
        if result["records"]:
            ...
            for rec in result["records"]:
                preview_rows.append({h: rec.get(h) for h in RAW_HEADERS[:23]})
            st.dataframe(pd.DataFrame(preview_rows), use_container_width=True)

        if result["failed_files"]:
            st.subheader("실패 파일")
            st.dataframe(
                pd.DataFrame(result["failed_files"], columns=["파일명", "reason"]),
        ...
        if result["excel_bytes"]:
            ...
            st.error("엑셀로 저장할 수 있는 PDF가 없습니다.")
